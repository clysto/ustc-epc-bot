...
class EPCBot(threading.Thread):
    ...
    URL_BOOKABLE = [
        {
            ...
            "url": URL_ROOT + "m_practice.asp?second_id=2001",
        },
        {
            "type": "Topical Discussion",
            "url": URL_ROOT + "m_practice.asp?second_id=2002",
        },
        {
            "type": "Debate",
            "url": URL_ROOT + "m_practice.asp?second_id=2003"
        },
        {
            "type": "Drama",
            "url": URL_ROOT + "m_practice.asp?second_id=2004"},
        {
            "type": "Pronunciation Practice",
            "url": URL_ROOT + "m_practice.asp?second_id=2007",
        }
    ]
    ...
    def login(self):
        ...
        if resp.status_code == 200 and not "登录失败" in resp.text:
    ...
    def submit_epc(self, epc_list: list, cmd: str):
        ...
        def foo(epc: dict):
            ...
            resp = self.session.post(
                url=self.URL_ROOT + epc["_url"], data=data)
    ...
    def optimize_epc(self, booked_epc: list, bookable_epc: list, hours_max: int):
        ...
        for epc in all_epc:
            ...
            # 如果课程单元重复, 跳过
            if epc["unit"] in [item["unit"] for item in optimal_epc]:
                continue

            # 如果课程时间重复, 跳过
            if epc["date"] in [item["date"] for item in optimal_epc]:
                continue

            # 检查边界条件
            hours_tmp = hours + int(epc["hour"])
            if hours_tmp > hours_max:
                continue
            hours = hours_tmp
    ...
    def list2html(self, epc_list: list):
        ...
        table.attrs = {"cellspacing": 0, "cellpadding": "4px", "border": 1}
